...
def getRandomNeighbour(coord):
    ...
    if ran_num < 1/4:   coord[0] = (coord[0] + 1) % VARIABLES["lattice_length"]
    elif ran_num < 1/2: coord[0] = (coord[0] - 1) % VARIABLES["lattice_length"]
    elif ran_num < 3/4: coord[1] = (coord[1] + 1) % VARIABLES["lattice_length"]
    else:   coord[1] = (coord[1] - 1) % VARIABLES["lattice_length"]
    return coord
