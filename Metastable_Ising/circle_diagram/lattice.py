...
class Lattice:
    ...
    def __init__(self, side_length, n_dim, S, initial_law, alpha=1, beta=4):
        ...
        elif "y" in initial_law:
            X = float(initial_law[1:])
            self.lattice_arr = self.getZeroYInitState(X)
        else:
            self.lattice_arr = initial_law*numpy.ones(shape=self.getLatticeShape(),dtype=int)

        self.initMagnetisation()    #vypnout, pokud neni metastableIsing
        self.initRates()


    def getZeroYInitState(self, X):
	       return numpy.random.choice([1,2,3,4], size = self.getLatticeShape(), p = [0.25 + X/4, 0.25 + X/4, 0.25 - X/4, 0.25 - X/4])
